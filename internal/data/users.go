...
var (
	pqDuplicateEmailStringEn = `pq: duplicate key value violates unique constraint "users_email_key"`
	pqDuplicateEmailStringRu = `pq: повторяющееся значение ключа нарушает ограничение уникальности "users_email_key"`
)
...
func ValidatePassword(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8, "password", "minimal length is 8")
	v.Check(len(password) <= 72, "password", "maximum length is 72")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "name", "must be provided")
	v.Check(len(user.Name) <= 500, "name", "maximum length is 500")
...
func (u UserModel) Insert(user *User) error {
	...
	if err != nil {
		switch {
		case err.Error() == pqDuplicateEmailStringRu:
